...
class ViewController: UITableViewController, UISearchBarDelegate {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row < voivodships.count && voivodships.count > 0 {
            ...
            let firstLetter = getSelectors(for: voivodships[indexPath.row]).first ?? ""
            cell.imageCOA.image = UIImage(named: firstLetter)
            return cell
        } else {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "segueSearchNew":
            if let destination = segue.destination as? NewDetailsViewController {
                destination.voivodship = selectedVoivodship
                destination.district = foundDistrict
                                    ...
                                    if let p = d[a] as? [String] {
                                        for s in p {
                                            if "\(w)\(s)" == term {
                                                ...
                                                performSegue(withIdentifier: "segueSearchNew", sender: self)
                                            }
                                        }
                                    }
                                }
